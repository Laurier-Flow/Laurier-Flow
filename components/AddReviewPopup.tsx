...
export default function AddReviewPopup({
    ...
    const submitNewReview = async (easy: number, useful: number, liked: number, instructor: string, text: string, courseName: string, user: User | null) => {
        if (isInstructor) {
            handleInstructorReviewSubmit(easy, useful, liked, instructor, text, courseName, user)
        } else {
            handleCourseReviewSubmit(easy, useful, liked, instructor, text, courseName, user)
        }
        onClose()
    }
        ...
        <div ref={popupRef} className="overflow-y-auto max-h-[90vh] border-2 border-slate-800 fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-background/80 backdrop-blur rounded-md max-w-xl w-11/12">
            ...
            >
                ...
                <div className='p-4 flex flex-col justify-between'>
                    <div className="flex flex-col flex-1 text-xl font-medium">
                        ...
                        <select value={instructor}
                            onChange={(e) => setInstructor(e.target.value)} id="hs-select-label" className="py-2 px-4 pe-9 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600">
                ...
                <div className='p-4 flex flex-row-reverse'>
                    <button type="submit" className="py-3 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-primary text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                        Submit
                    </button>
                </div>
            </form>
