...
async function InstructorSchedule({
    ...
    const termSections: sections = await getCourseSections(nextTermData, currentTermData, previousTermData, 'instructor_name_fk', instructorName, supabase, user)
    ...
    return (
        <div className="flex flex-col p-4 lg:mt-8">
            ...
            <ScheduleTable nextTerm={nextTerm} previousTerm={previousTerm} currentTerm={currentTerm} currentTermSections={currentTermSections} previousTermSections={previousTermSections} nextTermSections={nextTermSections} professor={true} user={user} />
        </div>
    )
