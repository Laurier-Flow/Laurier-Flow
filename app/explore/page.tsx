...
async function getCourses(
	...
	currentTerm: string,
	nextTerm: string
): Promise<courseInfoDBResponseExplore[]> {
	let sectionsPage = 0
	const limit = 1000
		...
		while (hasMoreSections) {
			...
			data.forEach(({ course_code_fk, term }) => {
				if (!courseOfferingsMap[course_code_fk]) {
					courseOfferingsMap[course_code_fk] = {
						isOfferedThisTerm: false,
						isOfferedNextTerm: false
					}
				}

				courseOfferingsMap[course_code_fk].isOfferedThisTerm ||=
					term === currentTerm
				courseOfferingsMap[course_code_fk].isOfferedNextTerm ||=
					term === nextTerm
			})
	...
	try {
		while (hasMoreCourses) {
			...
			const processedCourses: courseInfoDBResponseExplore[] = data.map(
				(course) => ({
					...course,
					isOfferedThisTerm:
						courseOfferingsMap[course.course_code]?.isOfferedThisTerm || false,
					isOfferedNextTerm:
						courseOfferingsMap[course.course_code]?.isOfferedNextTerm || false,
					easy:
						course.total_reviews > 0
							? Math.round((course.easy / course.total_reviews) * 20)
							: null,
					useful:
						course.total_reviews > 0
							? Math.round((course.useful / course.total_reviews) * 20)
							: null,
					liked:
						course.total_reviews > 0
							? Math.round((course.liked / course.total_reviews) * 100)
							: null
				})
			)
			...
			const processedInstructors = data.map((instructor) => ({
				...
				clear:
					instructor.total_reviews > 0
						? Math.round((instructor.clear / instructor.total_reviews) * 20)
						: 0,
				engaging:
					instructor.total_reviews > 0
						? Math.round((instructor.engaging / instructor.total_reviews) * 20)
						: 0,
				liked:
					instructor.total_reviews > 0
						? Math.round((instructor.liked / instructor.total_reviews) * 100)
						: 0,
				coursesTaught: Array.from(
					coursesTaughtMap.get(instructor.instructor_name) || []
				)
			}))
...
export default async function ExplorePage() {
	const [currentTerm, nextTerm, currentTermServer, nextTermServer] =
		await Promise.all([
			getCurrentTerm(true),
			getNextTerm(true),
